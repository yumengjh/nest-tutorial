...
拦截器（Interceptors）是 NestJS 中一个强大的功能，它就像是一个"中间人"，**可以在请求到达控制器的具体方法之前和响应发送给客户端之前进行拦截和处理**。通过 `@Injectable()` 装饰器注解并实现 `NestInterceptor` 接口来创建。

---

**拦截器的设计灵感来自 AOP（面向切面编程）**，它允许在不修改业务代码的情况下，添加横切关注点（cross-cutting concerns），如日志记录、性能监控、权限验证等。

---

拦截器可以实现以下功能：

**方法前后逻辑**：在请求处理前后注入自定义逻辑，如日志记录或请求验证。

**结果转换**：修改或统一控制器返回的数据格式。

**异常处理**：捕获并转换执行过程中抛出的异常，实现统一处理。

**功能扩展**：为现有功能添加额外行为，如修改响应头或状态码。

**条件执行**：根据特定条件决定是否执行原函数，如缓存或权限控制。

这些功能使得拦截器成为实现横切关注点的理想工具，能够以非侵入式的方式增强应用程序的功能。

## 基本{#basics}

每个拦截器需实现 `intercept()` 方法，接收两个参数：

**`ExecutionContext`**：继承自 `ArgumentsHost`，提供访问底层请求、响应等对象的能力，并根据应用类型（HTTP、RPC、WebSocket）暴露不同参数。可通过 `switchToHttp().getRequest()`、`switchToHttp().getResponse()` 或对应的 `switchToRpc()`、`switchToWs()` 获取原生对象。

同时增加了访问元数据的能力，可通过 `getHandler()` 获取当前处理方法，通过 `getClass()` 获取控制器类，便于基于元数据实现自定义逻辑或权限控制。

**`CallHandler`**：实现 `handle()` 方法，用于调用路由处理程序，必须调用才能执行后续处理。

简而言之，`ExecutionContext` 提供上下文与元数据访问，`CallHandler` 负责触发请求处理。
...
3. **getArgs()** - 返回当前处理函数的原始参数数组，比如可以访问控制器方法接收到的所有参数（例如 `req`, `res`, `next` 等）。

4. **getArgByIndex(index: number)** - 通过索引获取特定参数，比如当只想访问某个特定参数时，比如只取请求对象。

5. **switchToHttp()** - 切换到 HTTP 上下文，返回 `HttpArgumentsHost` 对象，提供 HTTP 请求和响应的便捷访问方法。

6. **switchToRpc()** - 切换到 RPC 上下文，返回 `RpcArgumentsHost` 对象，用于微服务或 RPC 场景。

7. **switchToWs()** - 切换到 WebSocket 上下文，返回 `WsArgumentsHost` 对象，用于 WebSocket 网关场景。

8. **getType<TContext = string>()** - 返回当前上下文类型，例如 `'http' | 'rpc' | 'ws'`，在拦截器或守卫里区分处理类型。
...
export class RateLimitInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    if (this.isRateLimited(clientId)) {
      // 超出限制，直接返回错误响应
      return throwError(() => new HttpException('请求过于频繁，请稍后再试', 429));
    }
...
在使用 RxJS 构建拦截器时，应根据具体需求选择合适的运算符，避免过度复杂化，同时注意性能和内存管理，确保 Observable 能正确完成，合理设置超时和重试策略。

在错误处理方面，应区分可重试和不可重试的错误，并提供有意义的错误信息。

日志记录要关注关键操作和上下文，并使用合适的日志级别。

通过编写单元测试验证复杂运算符组合、异常情况以及资源清理逻辑，可以让拦截器成为处理缓存、日志、QoS、错误等横切关注点的强大工具，从而构建更加健壮和高性能的应用程序。
